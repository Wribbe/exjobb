...
C = text.text(1*1.4, -1*0.5, r"\bf Test-run \#1", textattrs)
F = text.text(1*1.4, -2*0.5, r"\bf Test-run \#2", textattrs)
G = text.text(-0.3, -2*0.5, r"\bf Test-run \#3", textattrs)
H = text.text(-1*2.0, -2*0.5, r"\bf Test-run \#4", textattrs)
J = text.text(-1*2.0, -3*0.5, r"\bf Test-run \#5..n", textattrs)
...
for X in [C, D, E, F, G, H, J, K]:
    c.draw(X.bbox().enlarged(0.05).path(),
        [deco.stroked()])
    c.insert(X)


for X,Y in [[C, D], [D, E], [F, C], [G, F], [H, G], [J, H], [K, J]]:
