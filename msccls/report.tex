...
%			analysis.
	...
	\chapter{Evaluation}
		...
		\section{Results}
			...
			\subsection{Tests per user and outliers}
        ...
        \begin{figure}
          ...
          \caption{Tabulated values of test-run groupings and corresponding .}
        \end{figure}
      ...
      \subsection{Task type distribution}
        ...
        difference was not as stark with a more even distribution.
          ...
          \label{testsRunPerTaskOutliers}
				\end{figure}

        Observing the outlier distribution in figure
        \ref{testsRunPerTaskOutliers}, there is only two \numRuns{20} runs
      ...
      \subsection{Preferential task order}
        ...
        Splitting the grouped values into type-groups and dividing the type-sum
        with the total sum on that test-run index gives the percentage of that
        task type on the corresponding task-index, result presented below.
				\begin{figure}[ht!]
					\centering
					\includegraphics{figures/testsRunOrder.pdf}
          \caption{
            Percentage of task type grouped by test-run order for users that ran
            fifteen tests or less.
          }
				\end{figure}
				\begin{figure}[ht!]
					\centering
